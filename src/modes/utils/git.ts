...
// eslint-disable-next-line jsdoc/require-returns, jsdoc/require-param -- internal
/**
...
export const getRefData = <Q extends string>(
  ...
  const placeholdersIndexMap = Object.fromEntries(
    placeholders.map((placeholder, index) => [placeholder, index]),
  ...
  return result.map((r) => ({
    base: `/${r.name}/`,
    name: r.name,
    src: r.hash === currentCommit ? process.cwd() : path.join(srcRoot, r.name),
    data: r,
  }));
};
