...
import { zConfig } from "./modes.ts";

const CONFIG_PATHS =
  ".vitepress/{config/versions,versions,versions/index}.{js,ts,mjs,mts}" as const;

export default async (configPath: string | undefined) => {
  const resolvedPath = configPath ?? (await tinyglobby.glob(CONFIG_PATHS))[0];
